...
public interface IAirlineRepository extends JpaRepository<Airline,Integer> {
    ...
    List<Airline> findByTourPackage(String name) throws AirlineNotFoundException;

    @Query("from Airline a inner join a.packages t where t.cost<=?1")
    List<Airline> findByTourPackageLesserPrice(int price) throws AirlineNotFoundException;

    @Query("from Airline a inner join a.packages t where t.source=?1 and t.destination=?2")
    List<Airline> findBySourceAndDestination(String source,String destination) throws AirlineNotFoundException;
    ...
    List<Airline> findByYearOfOrigin(int yearOfOrigin) throws AirlineNotFoundException;

    @Query("from Airline a inner join a.details d where d.ranking=?1 and d.country=?2")
    List<Airline> findByRankingAndCountry(double ranking,String country) throws AirlineNotFoundException;
